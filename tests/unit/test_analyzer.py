...
def aws_calls(source_code):
    ...
    return analyzer.get_client_calls(real_source_code)


def chalice_aws_calls(source_code):
    real_source_code = dedent(source_code)
    return analyzer.get_client_calls_for_app(real_source_code)
...
def known_types_for_function(source_code, name):
    ...
    t = analyzer.SymbolTableTypeInfer(compiled)
    t.bind_types()
    return t.known_types(scope_name=name)
...
def test_mutual_recursion():
    assert aws_calls(
        ...
        a()
    """
    ) == {'ec2': {'describe_instances', 'run_instances'}}
...
def test_multiple_calls():
    ...
    """
    ) == {'dynamodb': {'list_tables', 'create_table'}}
...
def test_multiple_services():
    assert aws_calls(
        ...
        d.create_table(TableName='foobar')
    """
    ) == {'dynamodb': {'list_tables', 'create_table'}, 's3': {'get_object'}}
...
def test_multiple_targets():
    assert aws_calls(
        ...
        a.create_table()
    """
    ) == {'dynamodb': {'create_table', 'list_tables'}}
...
def test_map_string_literals_across_scopes():
    assert aws_calls(
        ...
        foo()
    """
    ) == {'s3': {'list_buckets'}, 'dynamodb': {'list_tables'}}
...
def test_can_analyze_combination():
    ...
    """
    ) == {'s3': {'list_buckets'}, 'ec2': {'describe_instances'}}
...
def test_can_handle_dict_comp_ifs():
    assert aws_calls(
        ...
        {x: y for x in d.create_table()\
         ...
         if d.list_tables()}
    """
    ) == {'dynamodb': {'list_tables', 'create_table', 'update_table'}}
